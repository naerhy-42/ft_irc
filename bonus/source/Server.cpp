...
namespace ft
{
	...
	int Server::init_bot(void)
	{
		if (!_bot.init(_port))
			return 0;
		_fds.push_back(_bot.get_socket());
		return 1;
	}
	...
	void Server::wait_connections(void)
	{
		...
		_bot.connect_to_server(_protocol.get_password());
		_bot.get_operator_privileges();
	    while (_server_status)
	    {
	        ...
	        else
	        {
				...
				else if (FD_ISSET(_bot.get_socket(), &read_fds))
				{
					ssize_t bytes_recv;

					memset(buffer, 0, _buffer_size);
					bytes_recv = recv(_bot.get_socket(), buffer, _buffer_size - 1, 0);
					_bot.parse_recv(std::string(buffer, bytes_recv));
				}
	            else
	            {
